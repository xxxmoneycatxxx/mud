...
// 组装 JSON 并写入文件（逐条编码避免单次 json_encode 过大）
void export_finalize()
{
    string json;
    string *keys;
    ...
    // 逐条编码每个主题，手动拼装外层 JSON
    keys = keys(export_topics);
    topics_json = "";
    for (i = 0; i < sizeof(keys); i++)
    {
        string entry_json = json_encode(export_topics[keys[i]]);
        if (topics_json != "") topics_json += ",";
        // key 需要 JSON 转义
        topics_json += "\"" + replace_string(replace_string(keys[i],
                      "\\", "\\\\"), "\"", "\\\"") + "\":" + entry_json;
    }

    json = sprintf("{\"ts\":%d,\"index\":\"%s\",\"topics\":{%s}}",
                   time(),
    ...
    if (write_file(HELP_OUTPUT, json, 1))
    {
        string msg = sprintf("帮助导出完成：%d 个主题 → %s (%d 字节)",
                              sizeof(export_files), HELP_OUTPUT, strlen(json));
    ...
    else
    {
        ...
        if (export_requestor) tell_object(export_requestor, "帮助导出失败：无法写入 " + HELP_OUTPUT + "\n");
    }
